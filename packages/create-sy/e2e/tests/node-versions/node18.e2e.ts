...
describe('Installation on Node.js 18', () => {
    ...
    it.skipIf(!container.isAvailable)(
        'verifies Node.js 18 environment',
        () => {
            const result = container.runInNode('18', [
                'node --version',
            ])

            expect(result.exitCode).toBe(0)
            expect(result.stdout).toMatch(/^v18\./)
        }
    )

    it.skipIf(!container.isAvailable)(
        'npm init sy@latest works on Node.js 18',
        () => {
            const result = container.runInNode('18', [
    ...
    it.skipIf(!container.isAvailable)(
        'syngrisi binary is available after installation on Node.js 18',
        () => {
            const result = container.runInNode('18', [
                'mkdir -p /tmp/test-project',
                'cd /tmp/test-project',
