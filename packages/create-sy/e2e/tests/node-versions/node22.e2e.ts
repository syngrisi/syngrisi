...
describe('Installation on Node.js 22', () => {
    ...
    it.skipIf(!container.isAvailable)(
        'verifies Node.js 22 environment',
        () => {
            const result = container.runInNode('22', [
                'node --version',
            ])

            expect(result.exitCode).toBe(0)
            expect(result.stdout).toMatch(/^v22\./)
        }
    )

    it.skipIf(!container.isAvailable)(
        'npm init sy@latest works on Node.js 22',
        () => {
            const result = container.runInNode('22', [
    ...
    it.skipIf(!container.isAvailable)(
        'syngrisi binary is available after installation on Node.js 22',
        () => {
            const result = container.runInNode('22', [
                'mkdir -p /tmp/test-project',
                'cd /tmp/test-project',
