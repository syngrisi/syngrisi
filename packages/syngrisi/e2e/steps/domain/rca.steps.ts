...
/**
 * Important CSS properties to collect for RCA
 */
...
/**
 * Script to collect DOM tree with computed styles (injected into browser)
 */
...
async function capturePageData(url: string): Promise<{ screenshot: Buffer; domDump: object }> {
    ...
    try {
        ...
        const domDump = await page.evaluate(DOM_COLLECTOR_SCRIPT);
